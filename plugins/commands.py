...
async def start(client, message):
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=user,text=f"<b>[ {get_size(files.file_size)} ] {files.file_name}\n\n📗 Download Link ➠ {g}</b>", reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('♻️ Download Link ♻️', url=g)
                    ], [
        ...
        if settings['is_shortlink'] and user not in PREMIUM_USER:
            ...
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
            k = await client.send_message(chat_id=message.from_user.id,text=f"<b>[{get_size(files.file_size)}] {files.file_name}\n\n📗 Download Link ➠ {g}</b>", reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton('♻️ Download Link ♻️', url=g)
                        ], [
